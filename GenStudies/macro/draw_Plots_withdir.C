...
void draw_Plots_withdir() {
  ...
  TFile* inFile1 = TFile::Open("/home/gouranga/Downloads/temp/MG24vs26/output_MG24x.root");
  TFile* inFile2 = TFile::Open("/home/gouranga/Downloads/temp/MG24vs26/output_MG26x.root");
  ...
  inFile1->cd("demo"); // interesting
  TList* list = gDirectory->GetListOfKeys() ; // interesting
  ...
  while ( (key = (TKey*)next()) ) {
    ...
    h_tmp = (TH1F*)inFile1->Get(abcd+obj->GetName());
    
    histos_f1.push_back(h_tmp);
  }

  inFile2->cd("demo"); // interesting
  list = gDirectory->GetListOfKeys() ; // interesting
  ...
  while ( (key = (TKey*)next2()) ) {
    ...
    printf("Histo name:%s title:%s\n",obj->GetName(),obj->GetTitle());
    if(obj->InheritsFrom("TTree")) continue;
    h_tmp = (TH1F*)inFile2->Get(abcd+obj->GetName());
...
void compareHistos(TH1F* h, TH1F* h2, string outputDir = "../output", string unit = "GeV", string label1 = "file1", string label2 = "file2")
{
     ...
     if(string(h->GetName()).find("pT_H")!=std::string::npos || string(h->GetName()).find("diHiggs_pt")!=std::string::npos){
       h->Rebin(2);
       h2->Rebin(2);
     }
     //cout << "h->GetYaxis()->Get"
